...
def tuned_build_ready_ranges():
    for pair in bot.PAIR_CONFIGS:
        if not pair_enabled(pair):
            continue
        window = bot.get_session_window(pair)
        if not window:
            continue
        if bot.now_utc() < window.range_end:
            continue
        if pair in bot.ranges and bot.ranges[pair]["trade_date"] == window.trade_date:
            continue
        built = bot.build_range_for_pair(pair)
        if built:
            bot.ranges[pair] = built
            if built.get("disabled"):
                bot.logger.info("%s disabled for %s: %s", pair, built["trade_date"], built.get("skip_reason", ""))


def tuned_fetch_live_prices() -> Dict[str, Dict[str, float]]:
    active_pairs = {pair for pair in bot.PAIR_CONFIGS if pair_enabled(pair)}
    active_pairs.update(str(trade["pair"]) for trade in bot.open_trades if trade["status"] == "OPEN")

    result: Dict[str, Dict[str, float]] = {}
    for pair in active_pairs:
        epic = bot.epics.get(pair)
        if not epic:
            continue
        live = bot.broker.get_live_price(epic)
        if live:
            result[pair] = live
        bot.time.sleep(0.2)
    return result
...
bot.build_ready_ranges = tuned_build_ready_ranges
bot.fetch_live_prices = tuned_fetch_live_prices
