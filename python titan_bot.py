...
class CapitalClient:
    ...
    def place_order(self, pair: str, epic: str, direction: str, units: float,
        ...
        # ── SL-only order — TP fully managed by bot ──────────────────────
        # Capital.com API does not reliably accept limitLevel on this account.
        # Bot monitors price every 20s and closes trade when TP is hit.
        # SL stays on broker as safety net if bot goes offline.
        payload = {
            "epic":           epic,
            "direction":      direction,
            "size":           float(units),
        ...
        r1 = self._req("POST", "/api/v1/positions", payload)
        logger.info(f"Order response: {r1}")

        if r1 and r1.get("dealReference"):
            logger.info(f"✅ Order placed | SL={float(sl_r)} | TP={float(tp_r)} (bot-managed) | ref={r1['dealReference']}")
            return r1["dealReference"]

        logger.error(f"❌ Order failed for {epic}")
    ...
    def confirm_fill(self, deal_ref: str) -> Optional[Dict]:
        """
        Confirms the order was filled by Capital.com.
        TP is now managed entirely by the bot — no broker TP needed.
        """
        time.sleep(2)
