...
class CapitalClient:
    ...
    def confirm_fill(self, deal_ref: str, tp_level: float = None, sl_level: float = None) -> Optional[Dict]:
        """
        Confirms fill and auto-fixes missing TP via PUT if needed.
        Root cause of TP bug: Capital.com silently ignores limitLevel
        when decimal precision does not match instrument requirements.
        This method detects and fixes it automatically.
        """
        ...
        if status not in ["OPEN", "ACCEPTED"]:
            logger.warning(f"Fill status unexpected: {status}")
            return None

        # Check TP in confirm response
        if data.get("limitLevel"):
            logger.info(f"✅ TP confirmed: {data['limitLevel']}")
            return data

        # TP missing — check positions and auto-fix
        ...
        for pos in positions.get("positions", []):
            ...
            logger.info(f"Position check: dealId={deal_id} TP={limit}")

            if limit:
                logger.info(f"✅ TP found via positions: {limit}")
            ...
            if tp_level and deal_id:
                ...
                logger.info(f"PUT TP result: {result}")
                if result:
                    logger.info(f"✅ TP auto-fixed via PUT: {tp_level}")
                    data["limitLevel"] = tp_level
                    send_telegram(f"✅ TP auto-fixed!\nref={deal_ref} TP={tp_level}")
                else:
                    logger.error(f"❌ PUT TP failed for {deal_id}")
                    send_telegram(
                        f"⚠️ TP could not be auto-fixed!\n"
                        f"Please set TP={tp_level} manually on Capital.com\n"
                        f"ref={deal_ref}"
                    )
            break
