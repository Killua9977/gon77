...
class CapitalClient:
    ...
    def _resolve(self, identifier: str) -> Optional[Dict]:
    ...
    def close_position(self, identifier: str) -> bool:
        pos = self._resolve(identifier)
        if not pos:
            logger.warning("Position not found: %s — treating as closed",
                           identifier)
            return True
        r = self._req("DELETE", f"/api/v1/positions/{pos['dealId']}")
...
def finalize_trade(trade: Dict, exit_price: float,
    ...
    if broker_close and broker:
        ...
        if identifier:
            broker.close_position(identifier)
